 ...
 * =============================================================================
 * 
 * Read-only view of student's attendance history.
 * =============================================================================
 */

import React, { useState } from 'react';
import { 
  FiCalendar, FiCheckCircle, FiClock, FiXCircle, FiFilter
} from 'react-icons/fi';
...
export const LearnerAttendance: React.FC = () => {
  ...
  // Calculate stats
  const allAttendance = mockAttendance.filter(a => a.studentId === student.studentId);
        ...
        <div className="bg-card rounded-xl border border-border p-4">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 rounded-lg bg-destructive/10 flex items-center justify-center">
              <FiXCircle className="w-5 h-5 text-destructive" />
            </div>
          </div>
        ...
        <div className="bg-card rounded-xl border border-border p-4">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 rounded-lg bg-accent/10 flex items-center justify-center">
              <FiCalendar className="w-5 h-5 text-accent-foreground" />
            </div>
          </div>
      ...
      <div className="bg-card rounded-2xl border border-border shadow-soft overflow-hidden">
              ...
              {studentAttendance.map((record) => {
                ...
                return (
                  <tr key={record.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-foreground">{dayName}</td>
                    <td className="px-6 py-4">
                      <Badge variant={
        ...
        {studentAttendance.length === 0 && (
          <div className="p-12 text-center">
            <FiCalendar className="w-12 h-12 mx-auto text-muted-foreground/50 mb-4" />
