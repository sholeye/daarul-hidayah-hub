/**
 * =============================================================================
 * INSTRUCTOR RESULTS PAGE - Enter/Edit scores for students
 * =============================================================================
 */

import React, { useState } from 'react';
import { FiFileText, FiSave, FiCheck, FiX } from 'react-icons/fi';
import { mockStudents, mockResults, schoolClasses } from '@/data/mockData';
...
export const InstructorResults: React.FC = () => {
  ...
  const handleSave = () => {
    ...
    const existingIndex = results.findIndex(r => r.studentId === selectedStudent.studentId);
    if (existingIndex >= 0) {
      setResults(results.map((r, i) => i === existingIndex ? newResult : r));
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-foreground">Enter Results</h1>
        <p className="text-muted-foreground mt-1">Enter scores for students in your classes</p>
      </div>
              ...
              return (
                <button key={student.id} onClick={() => handleSelectStudent(student)}
                  ...
                  <div className="text-left flex-1">
                    ...
                    <p className={`text-sm ${selectedStudent?.id === student.id ? 'opacity-75' : 'text-muted-foreground'}`}>{student.studentId}</p>
                  </div>
          ...
          {selectedStudent ? (
            <>
              ...
              <div className="space-y-4 max-h-[400px] overflow-y-auto">
                  ...
                  return (
                    <div key={subject} className="flex items-center gap-4">
                      <div className="flex-1">
                        ...
                        <Input type="number" min="0" max="100" value={scores[subject] || ''} onChange={(e) => setScores({ ...scores, [subject]: parseInt(e.target.value) || 0 })} placeholder="0" />
                      </div>
                ...
                <div>
                  <label className="text-sm text-muted-foreground">Position in Class</label>
                  <Input type="number" min="1" value={position} onChange={(e) => setPosition(e.target.value)} />
                </div>
                <div>
                  <label className="text-sm text-muted-foreground">Teacher's Remarks</label>
                  <textarea value={teacherRemarks} onChange={(e) => setTeacherRemarks(e.target.value)} className="w-full h-20 px-3 py-2 rounded-lg border border-input bg-background text-foreground resize-none" placeholder="Enter remarks..." />
                </div>
              </div>
            </>
          ) : (
            <div className="h-full flex items-center justify-center text-center p-12">
              <div><FiFileText className="w-12 h-12 mx-auto text-muted-foreground/50 mb-4" /><p className="text-muted-foreground">Select a student to enter results</p></div>
            </div>
          )}
        </div>
      </div>
